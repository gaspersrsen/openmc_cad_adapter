        ...
        if isinstance(node, Halfspace):
            ...
            for subnode in node:
                ...
                if stp + 1 != last_id(body_id()): # If multiple volumes are created they are saves as a multivolume body
                    exec_cubit( f"split body {' '.join( map(str, np.array(inter_id)+1) )}" ) # Split the multivolume body
                inter_id = body_id()
            ...
            if first + 1 != last_id(body_id()): # If multiple volumes are created they are saves as a multivolume body
                exec_cubit( f"split body {' '.join( map(str, np.array(union_id)+1) )}" ) # Split the multivolume body
            union_id = body_id()
