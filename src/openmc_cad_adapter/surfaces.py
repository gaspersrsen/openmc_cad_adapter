...
class CADXPlane(CADSurface, openmc.XPlane):
    ...
    def to_cubit_surface_inner(self, ent_type, node, extents, inner_world=None, hex=False, off_center=0):
        ...
        if surf_id(node) not in surf_map:
            ...
            ids_map = body_id()
            exec_cubit(f"section volume {{ {ids_map} }} with xplane offset {self.coefficients['x0']-extents[0]/2*off_center} {self.reverse(node)}")
...
class CADYPlane(CADSurface, openmc.YPlane):
    ...
    def to_cubit_surface_inner(self, ent_type, node, extents, inner_world=None, hex=False, off_center=0):
        ...
        if surf_id(node) not in surf_map:
            exec_cubit(f"brick x {extents[0]} y {extents[1]} z {extents[2]}")
            id_map = body_id()
            exec_cubit(f"section volume {{ {id_map} }} with yplane offset {self.coefficients['y0']-extents[1]/2*off_center} {self.reverse(node)}")
